...
Codex review is the default when no engine is set. It is locked to `gpt-5.5` with `high` reasoning and should remain the normal final closeout engine. Claude review is optional and uses `claude-fable-5` by default.
...
If the native Codex CLI is installed through the Windows app package and fails with `Access is denied`, run the Codex engine from WSL instead of the native Windows shell.
